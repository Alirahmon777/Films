...
function renderPlan(plan) {
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    const resultTime = getUserTime(plan[i].release_date * 1000);

    newItem.className = "film__items";

    newItem.innerHTML = `
  <div class="film__imgs">
    <img
      src="${plan[i].poster}"
      width="200"
      alt="Shazam film"
  ...
  <div class="film__about">
    <h3 class="film__title">${plan[i].title}</h3>
    <p class="film__text">${plan[i].overview}</p>
    <p class="film__date">${resultTime}</p>
    <p class="film__genre">${plan[i].genres}</p>
  </div>`;
